...
function JoinEventPage() {
  ...
  const handleSearch = async () => {
    ...
    if (!trimmed) return
    const event = await getEventByCode({ data: trimmed })
    if (event) {
      setFoundEvent(event)
      setStatus('found')
    } else {
    ...
    <main className="page-wrap px-4 py-4">
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2">
          <input type="text" value={code} onChange={(e) => { setCode(e.target.value.toUpperCase()); setStatus('idle') }} onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
          ...
          <button onClick={handleSearch} className="flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--mag-green)] text-white transition hover:bg-[var(--mag-green-dark)]">
      ...
      <div className="mb-6 rounded-2xl border border-dashed border-[var(--mag-line)] bg-[var(--mag-card)] p-6 text-center">
        ...
        <p className="mt-1 text-xs text-[var(--mag-ink-soft)]">Point your camera at an event QR code to join instantly.</p>
      </div>
        ...
        <div className="rounded-2xl border border-[var(--mag-green)] bg-[var(--mag-green)]/5 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-[10px] text-[var(--mag-ink-muted)]">{foundEvent.location}</p>
            </div>
          </div>
          <button onClick={handleJoin} className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-[var(--mag-green)] py-2.5 text-xs font-bold text-white transition hover:bg-[var(--mag-green-dark)]">Join Event</button>
        </div>
      )}
