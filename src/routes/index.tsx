...
function LandingPage() {
  return (
    <main className="page-wrap flex min-h-[80vh] flex-col items-center justify-center px-4 py-12 text-center">
      ...
      <p className="rise-in mb-8 max-w-sm text-lg text-[var(--mag-ink-soft)]" style={{ animationDelay: '160ms' }}>
      ...
      <div className="rise-in flex w-full max-w-xs flex-col gap-3" style={{ animationDelay: '320ms' }}>
        <Link
          to="/login"
          className="flex items-center justify-center rounded-full bg-[var(--mag-ink)] px-6 py-3.5 text-lg font-semibold !text-[var(--mag-bg)] transition hover:opacity-80 active:scale-95 no-underline"
        >
          Get started
        </Link>
      </div>

      <p className="rise-in mt-8 text-sm text-[var(--mag-ink-muted)]" style={{ animationDelay: '400ms' }}>
