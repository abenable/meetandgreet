  ...
  return (
    <main className="page-wrap px-4 py-4 pb-nav">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-center text-2xl font-bold text-[var(--mag-ink)]">Top Picks</h1>
      ...
      <p className="mb-4 text-sm text-[var(--mag-ink-soft)]">Curated profiles selected just for you.</p>

      <div className="grid grid-cols-2 gap-3">
        {picks.map((profile, i) => (
          <div key={i} className="overflow-hidden rounded-2xl bg-[var(--mag-card)] shadow-sm">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3">
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {profile.tags.slice(0, 2).map((tag) => (
                    <span key={tag} className="rounded-full bg-white/20 px-2 py-0.5 text-xs font-medium text-white backdrop-blur-sm">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
