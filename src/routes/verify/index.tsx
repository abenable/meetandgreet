...
function VerifyIntroPage() {
  return (
    <main className="page-wrap flex min-h-[80vh] flex-col items-center justify-center px-4 py-8 text-center">
      <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-[var(--mag-green)]/10">
        <Shield className="h-10 w-10 text-[var(--mag-green)]" />
      </div>
      ...
      <div className="mt-8 w-full max-w-xs space-y-3 text-left">
        {[
          ...
          <div key={i} className="flex items-start gap-3 rounded-xl border border-[var(--mag-line)] bg-[var(--mag-card)] p-3">
            <CheckCircle className="mt-0.5 h-4 w-4 flex-shrink-0 text-[var(--mag-green)]" />
      ...
      <Link
        to="/verify/capture"
        className="mt-8 flex items-center justify-center gap-2 rounded-full bg-[var(--mag-green)] px-8 py-3.5 text-sm font-bold text-white shadow-md transition hover:bg-[var(--mag-green-dark)] no-underline"
      >
        Start Verification
